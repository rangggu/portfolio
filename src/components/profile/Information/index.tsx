...
export default function Information({ isVisible }: Props) {
  ...
  const { name, profile, phone, email, github } = PROFILE
  ...
  return (
    ...
    >
      <motion.div className="flex flex-col items-center w-full text-h3 font-semibold">
        ...
        <h2 className="tracking-[0.12em] leading-tight">{name.en}</h2>
      </motion.div>
      ...
      <motion.div className="flex flex-col gap-4 w-full text-body1 tracking-wider">
        ...
        <div className="flex items-center gap-5">
          ...
          <Link className="underline" href={github} rel="noopener noreferrer" target="_blank">
            링크
          </Link>
        </div>
      </motion.div>
